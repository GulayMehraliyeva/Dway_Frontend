...
// ── shared store so the toggle in Profile is immediately visible to index.tsx ──
...
export default function ProfileScreen() {
  ...
  const handleSaveName = () => {
    setData((prev) => prev ? { ...prev, user: { ...prev.user, name: editedName } } : prev);
    setShowEditName(false);
  };

  /**
   * Toggle push notifications.
   * Writes to the shared notificationStore so any screen that reads from it
   * (e.g. index.tsx via loadProfileSettings) always gets the current value.
   */
  const handlePushNotificationsChange = async (value: boolean) => {
    setPushNotifications(value);
    notificationStore.set(value);       // ← update the shared store immediately
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            setData((prev) =>
              prev ? { ...prev, stats: { tasksDone: 0, dayStreak: 0, completion: 0 } } : prev
            );
          },
        },
      ]
  ...
  const loadData = async () => {
    ...
    setData(result);
    // Sync UI state AND the shared store from the persisted profile value
    const savedPush = result.notifications.pushNotifications;
    setPushNotifications(savedPush);
    notificationStore.set(savedPush);   // ← keep store in sync on load too
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.profileSection}>
          ...
          <View style={styles.nameRow}>
            ...
            <TouchableOpacity onPress={() => { setEditedName(data.user.name); setShowEditName(true); }}>
        ...
        <View style={styles.achievementContainer}>
          <View style={styles.achievementCard}>
            <View style={styles.iconCircleOrange}><FlashIcon width={18} height={18} /></View>
            <Text style={styles.achievementText}>{data.achievements.first}</Text>
          </View>
          <View style={styles.achievementCard}>
            <View style={styles.iconCircleGreen}><DoneIcon width={18} height={18} /></View>
            <Text style={styles.achievementText}>{data.achievements.second}</Text>
          </View>
          <View style={styles.achievementCard}>
            <View style={styles.iconCirclePurple}><ClubIcon width={18} height={18} /></View>
        ...
        <View style={styles.communityCard}>
          ...
          <Text style={styles.communityText}>Visit Community to join one!</Text>
        </View>

        {/* ── Notifications ── */}
        <Text style={styles.sectionTitle}>NOTIFICATIONS</Text>
        <View style={styles.notificationCard}>
          {/* Push Notifications toggle — controls whether reminders are sent */}
        ...
        <View style={styles.settingsCard}>
          ...
          >
            <View style={styles.leftContent}>
              <View style={styles.iconBoxBlue}><SunIcon width={23} height={23} /></View>
        ...
        <View style={styles.partnerCard}>
          ...
          <TouchableOpacity style={styles.partnerButton} onPress={() => Linking.openURL("https://www.headspace.com")}>
        ...
        <View style={styles.partnerCard}>
          ...
          <View style={styles.partnerDivider} />
          <TouchableOpacity style={styles.partnerButton} onPress={() => Linking.openURL("https://www.coursera.org")}>
        ...
        <View style={styles.dataCard}>
          <TouchableOpacity style={styles.dataRow} onPress={handleClearData} activeOpacity={0.7}>
            <View style={styles.leftContent}>
              <View style={styles.deleteIconBox}><TrashIcon width={18} height={18} /></View>
          ...
          <TouchableOpacity style={styles.dataRow} onPress={handleLogout} activeOpacity={0.7}>
            <View style={styles.leftContent}>
              <View style={styles.logoutIconBox}><LogoutIcon width={18} height={18} /></View>
      ...
      <Modal visible={showEditName} transparent animationType="slide" onRequestClose={() => setShowEditName(false)}>
        <KeyboardAvoidingView style={styles.editModal} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <TouchableOpacity style={styles.editOverlay} activeOpacity={1} onPress={() => setShowEditName(false)}>
      ...
      {/* ── Alarm Sound sheet ── */}
      <Modal visible={showAlarmSheet} transparent animationType="slide" onRequestClose={() => setShowAlarmSheet(false)}>
        <View style={styles.sheetOverlay}>
          <TouchableOpacity style={styles.overlayBackground} activeOpacity={1} onPress={() => setShowAlarmSheet(false)} />
          <View style={styles.bottomSheet}>
            ...
            {["Standard alarm sound","Soft, gradual chime","Classic beep tone","Clear bell ring"].map((sound) => (
              ...
              >
                <Text style={[styles.soundText, selectedAlarmSound === sound && { color: "#5E5CE6" }]}>{sound}</Text>
      ...
      {/* ── Theme sheet ── */}
      <Modal visible={showThemeSheet} transparent animationType="slide" onRequestClose={() => setShowThemeSheet(false)}>
        <View style={styles.sheetOverlay}>
